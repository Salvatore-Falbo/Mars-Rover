...
public class BackwardCommand implements CommandInterface {
    ...
    public void execute() throws ObstacleDetectedException {
        ...
        int nextX = (RoverState.getPosition().getX() + Constants.GRID_SIZE + (int)Math.cos(RoverState.getAngle() + Math.PI)) % Constants.GRID_SIZE;
        int nextY = (RoverState.getPosition().getY() + Constants.GRID_SIZE + (int)Math.sin(RoverState.getAngle() + Math.PI)) % Constants.GRID_SIZE;
